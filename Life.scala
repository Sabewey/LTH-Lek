...
case class Life(cells: Matrix[Boolean]) {
  ...
  /** Ger true om cellen på plats (row, col) är vid liv annars false.
    ...
    */
  def apply(row: Int, col: Int): Boolean = if (row >= 0 && row < cells.dim._1 && col >= 0 && col < cells.dim._2) cells(row, col) else false
  //if (row <= cells.dim._1 - 1 && row > 0 && col <= cells.dim._2 - 1 && col > 0) cells(row, col) else false
...
object Life {
  ...
  def defaultRule(row: Int, col: Int, current: Life): Boolean = {
    //Om den lever...
    if (current.cells(row, col) && (current.nbrOfNeighbours(row, col) == 2 || current.nbrOfNeighbours(row, col) == 3)) {
      true
    } else if (!current.cells(row, col) && current.nbrOfNeighbours(row, col) == 3){
      true
    } else false 
  }
}
